...
const string strMessageMagic = "Two Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
   ...
   if (pindexBest->nHeight <= 10)
      {
        int64_t nSubsidy = 250000 * COIN;
        return nSubsidy + nFees;
      }
      
    else if (pindexBest->nHeight <= 30)
      {
        int64_t nSubsidy = 1000 * COIN;
        return nSubsidy + nFees;
      }
	  
	else if (pindexBest->nHeight <= 470)
      {
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Two", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
